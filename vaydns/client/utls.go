...
// handshake with the provided ClientHelloID, and returns the resulting TLS
// connection.
func UTLSDialContext(ctx context.Context, network, addr string, config *utls.Config, id *utls.ClientHelloID, control func(network, address string, c syscall.RawConn) error) (*utls.UConn, error) { //Added
...
//  Passing the Android protector to the TCP Dialer
	dialer := &net.Dialer{Control: control}	//Aded
//	dialer := &net.Dialer{}
...
type utlsRoundTripper struct {
	...
	config        *utls.Config
	control       func(network, address string, c syscall.RawConn) error // Added
...
func NewUTLSRoundTripper(config *utls.Config, id *utls.ClientHelloID, control func(network, address string, c syscall.RawConn) error) *utlsRoundTripper { // Added
	return &utlsRoundTripper{
		clientHelloID: id,
		config:        config,
		control:       control, // Added
...
//		rt.inner, err = makeRoundTripper(req, rt.config, rt.clientHelloID)
		rt.inner, err = makeRoundTripper(req, rt.config, rt.clientHelloID, rt.control) //Added
	}
	rt.innerLock.Unlock()
	if err != nil {
		return nil, err
...
// ClientHelloID.
func makeRoundTripper(req *http.Request, config *utls.Config, id *utls.ClientHelloID, control func(network, address string, c syscall.RawConn) error) (http.RoundTripper, error) { // Added
//func makeRoundTripper(req *http.Request, config *utls.Config, id *utls.ClientHelloID) (http.RoundTripper, error) {
	...
	bootstrapConn, err := UTLSDialContext(req.Context(), "tcp", addr, config, id, control) // Added
//	bootstrapConn, err := UTLSDialContext(req.Context(), "tcp", addr, config, id)
	...
	dialTLSContext := func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		uconn, err := UTLSDialContext(ctx, "tcp", addr, config, id, control) // Added
