...
def main():
    ...
    # SSH Port handling options (Fixing the connection bug)
    current_ssh_port_input = input("Current SSH Port (default: 22): ").strip()
    current_ssh_port = int(current_ssh_port_input) if current_ssh_port_input.isdigit() else 22
    
    change_ssh_port = input("Do you want to change the SSH port? (y/N) [default: no]: ").strip().lower()
    ...
    # Connect to Server using the CURRENT SSH port so it doesn't fail
    ...
    if replace_dns_bool:
        ...
        # Remove immutable flag temporarily in case the file was previously locked
        run_cmd(ssh, "chattr -i /etc/resolv.conf || true", user, password, hide_output=True)
        
        # Read the current contents of resolv.conf
        ...
        for line in resolv_current.splitlines():
            ...
            if line_stripped.startswith("nameserver"):
                # If the script was run previously, avoid duplicating the new DNS servers
                if "8.8.8.8" in line_stripped or "1.1.1.1" in line_stripped:
                    continue
                # Comment out old nameservers
                new_lines.append("# " + line_stripped)
            else:
                # Keep comments and other configurations intact
                new_lines.append(line_stripped)
                
        # Append the new DNS servers at the bottom
        ...
        # Lock the file and reload NetworkManager
    ...
    if is_ubuntu: 
        ...
        run_cmd(ssh, "export DEBIAN_FRONTEND=noninteractive && apt-get install tar dante-server ufw curl vnstat sed tcpdump net-tools bind9-dnsutils policycoreutils -y", user, password)
    else:
    ...
    if is_ubuntu:
        ...
        # Explicitly open the NEW target port in the firewall ahead of time
        run_cmd(ssh, f"ufw allow {new_ssh_port}/tcp", user, password)  
...
socks pass {
    ...
    if current_ssh_port != new_ssh_port:
        ...
        # Remove old firewall rules to lock down the old port
        if is_ubuntu:
            if current_ssh_port == 22:
                run_cmd(ssh, "ufw delete allow 22/tcp || true", user, password)
                run_cmd(ssh, "ufw delete allow ssh || true", user, password)
            else:
                run_cmd(ssh, f"ufw delete allow {current_ssh_port}/tcp || true", user, password)
        else:
            if current_ssh_port == 22:
        ...
        # Safely overwrite or append the new Port parameter in sshd_config
