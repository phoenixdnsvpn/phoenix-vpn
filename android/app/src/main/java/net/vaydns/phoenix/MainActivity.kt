...
// Read the actual connected config, fallback to selected if missing
    ...
    private fun initDefaultSettings() {
        ...
        if (!appPrefs.contains("default_configs_at_start")) {
            appPrefs.edit().apply {
                ...
                // FALSE: Hide legacy/pure Phoenix configs by default
    ...
    private val requestNotificationPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted: Boolean ->
            if (isGranted) {
                Log.i("Phoenix", "Notification permission granted by user.")
            } else {
                Toast.makeText(
                    this,
                    "Notifications disabled. You will not see the active VPN status icon in your status bar.",
                    Toast.LENGTH_LONG
                ).show()
            }

            // CHAINING: Trigger the Battery App Info dialog immediately after they answer
    ...
    private fun askForNotificationPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            if (!hasAsked && ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
                ...
                // If we already asked for notifications in the past, or they already granted it, skip straight to battery
                checkBatteryOptimizations()
            }
        } else {
            // Android 12 and below don't need notification prompts, jump straight to battery
            checkBatteryOptimizations()
        }
    }
            ...
            if (!isIgnoringOptimizations) {
                ...
                if (!hasAskedBattery) {
                    ...
                    MaterialAlertDialogBuilder(this)
                        .setTitle("Background Connection / اتصال در پس‌زمینه")
                        .setMessage("To prevent the VPN from disconnecting when your screen is off, you must enable one setting manually:\n\n" +
                                "1. Tap 'Open Settings' below.\n" +
                                "2. Tap on 'Battery' (or Battery usage).\n" +
                                "3. Enable 'Allow background activity' or disable restrictions.\n" +
                                "--------------------------------------------------\n" +
                                "برای جلوگیری از قطع شدن VPN هنگام خاموش شدن صفحه، باید یک تنظیم را به صورت دستی فعال کنید:\n\n" +
                                "۱. روی «باز کردن تنظیمات» در پایین بزنید.\n" +
                                "۲. روی گزینه «Battery» (باتری) یا «Battery usage» بزنید.\n" +
                                "۳. گزینه فعالیت در پس‌زمینه را مجاز کنید یا محدودیت را بردارید.")
                        .setPositiveButton("Open Settings / باز کردن تنظیمات") { dialog, _ ->
                            dialog.dismiss()

                            // Delay slightly to prevent UI glitching, exactly like your Scanner
                            Handler(Looper.getMainLooper()).postDelayed({
                                try {
                                    // Takes the user to App Info page (highly compatible with Realme/Xiaomi)
...
// Fetch the Vault JSON
            ...
            if (isDirectMode) {
                if (cleanProtocol.lowercase() == "vless-ws" || cleanProtocol.lowercase() == "vless-httpupgrade") {
                    ...
                    // Custom Hysteria/Reality configs use the DNS field for their IP
                    serverIp = finalConfig.dnsAddress
                }
                // Note: Default Hysteria/Reality configs leave serverIp as "" so Go fetches it from its internal native vault.
            }
...
// Use android.R.attr to point specifically to the theme attributes
        ...
        return when (item.itemId) {
            ...
            R.id.action_cf_scanner -> {
                ...
                if (rawConfig == null) {
                    Toast.makeText(
                        this,
                        "Please select a config first. The scanner requires a domain to test.",
                        Toast.LENGTH_LONG
                    ).show()
                    return true
                }
            ...
            R.id.action_global_dns_scanner -> {
                // 1. Guardrail: Force the user to stop the VPN to get accurate raw latencies
                if (isVpnConnected) {
                    Toast.makeText(this, "Please stop the VPN before running a scan.", Toast.LENGTH_LONG).show()
                    return true
                }
    ...
    private fun checkForAppUpdate() {
        ...
        Thread {
            try {
                // 1. Ask Go which release track we are on
                val releaseType = mobile.Mobile.getReleaseType()

                if (releaseType.lowercase() == "private") {
                    // ==========================================
                    // PRIVATE RELEASE (Check Custom Server via Go)
                    // ==========================================
                    // This uses your existing CheckForAppUpdate in updater.go
                    val result = mobile.Mobile.checkForAppUpdate(currentVersion)

                    runOnUiThread {
                        if (result.startsWith("UPDATE_AVAILABLE|")) {
                            val newVersion = result.substringAfter("|")
                            showUpdateDialog(newVersion, releaseType)
                            ...
                            Toast.makeText(this@MainActivity, "Update check failed: $result", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    // ==========================================
                    ...
                    if (connection.responseCode == java.net.HttpURLConnection.HTTP_OK) {
                        ...
                        runOnUiThread {
                            if (isNewerAppVersionLocal(cleanCurrent, fetchedVersion)) {
                                showUpdateDialog(fetchedVersion, releaseType)
                            } else {
                                Toast.makeText(this@MainActivity, "You are using the latest version ($currentVersion).", Toast.LENGTH_SHORT).show()
                            }
                        }
                ...
                e.printStackTrace()
                runOnUiThread {
                    Toast.makeText(this@MainActivity, "Failed to connect to update server.", Toast.LENGTH_SHORT).show()
                }
            }
        }.start()
    }
...
// Changed signature to accept the releaseType
    private fun showUpdateDialog(newVersion: String, releaseType: String = "community") {
        runOnUiThread {
            ...
            MaterialAlertDialogBuilder(this)
                .setTitle("App Update Available")
                .setMessage("A new version of Phoenix VPN ($versionStr) is available.\n\n" +
                        "Note: Your browser may say 'File might be harmful' because this is a direct APK download. It is safe to tap 'Download anyway'. If file download progress was not displayed, please check your Downloads directory or Notification panel.\n\n" +
        ...
        if (checkWarning && activeProtocol.lowercase() == "vaydns" && selectedApps.size > 5) {
            ...
            if (!prefs.getBoolean("hide_vaydns_app_warning", false)) {
                ...
                showVayDnsAppWarningDialog {
                    // If the user clicks "Continue", resume the tunnel startup sequence silently
                    startVpnService(checkWarning = false)
                }
                return // Abort the current execution loop to wait for the dialog
            }
        }
...
//            val tunnelPrefs = getSharedPreferences("TunnelSettingsPrefs", Context.MODE_PRIVATE)
    ...
    private fun showVayDnsAppWarningDialog(onContinue: () -> Unit) {
        val container = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            val pad = (20 * resources.displayMetrics.density).toInt()
        ...
        val onSurfaceColor = MaterialColors.getColor(this, com.google.android.material.R.attr.colorOnSurface, Color.BLACK)

        val tvWarning = TextView(this).apply {
            text = "You have selected more than 5 applications to tunnel.\n\nThe vaydns protocol is designed for bypassing severe censorship, which means it has limited bandwidth. Routing too many apps through it may cause significant slowdowns or connection instability.\n\nشما بیش از ۵ برنامه را برای عبور از تونل انتخاب کرده‌اید. پروتکل Phoenix دارای پهنای باند محدودی است. عبور دادن برنامه‌های زیاد می‌تواند باعث کندی شدید یا ناپایداری اتصال شود."
            textSize = 14f
            setTextColor(onSurfaceColor)
            setLineSpacing(0f, 1.2f)
        }

        val cbDontShowAgain = CheckBox(this).apply {
            text = "Do not display this warning / دیگر نشان نده"
            textSize = 13f
            setTextColor(onSurfaceColor)
        ...
        container.addView(cbDontShowAgain)

        MaterialAlertDialogBuilder(this)
            .setTitle("Bandwidth Warning")
            .setView(container)
