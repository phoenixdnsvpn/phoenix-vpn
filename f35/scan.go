...
func checkResolver(ctx context.Context, client *http.Client, cfg *runtimeConfig, resolver parsedResolver, port int) (Result, bool) {
	...
	// ULTRADENSE ROUTER PROTECTION: Increased pacing cooldown to 100ms
	defer func() {
		if smuxUsed {
			// Full E2E mode with active HTTP traffic stream flush delays
			go func() {
			...
			// Light E2E/Handshake closes sockets synchronously. 
			// This completely eliminates NAT conntrack table saturation on fragile home gateways.
			cancelTunnel()
