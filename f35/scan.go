...
//	performHandshake := tCfg.EngineQuickScan || (tCfg.LightE2EEnabled && !dnsPassed)
...
func checkResolver4(ctx context.Context, client *http.Client, cfg *runtimeConfig, resolver parsedResolver, port int) (Result, bool) {
...
//	uniquePort := int(atomic.AddInt32(&dynamicPort, 1))
	...
	if performHandshake {
		...
		// COMPILE FIX: Pass tCfg into the handshake checker!
		if !verifyTunnelHandshake(ctx, port, tCfg) {
			return Result{
				Resolver: resolver.addr, 
				Probe: "h-fail", 
...
func checkResolver3(ctx context.Context, client *http.Client, cfg *runtimeConfig, resolver parsedResolver, port int) (Result, bool) {
    ...
    //  FIX 1: Create a context SPECIFIC to this single tunnel
	tunnelCtx, cancelTunnel := context.WithCancel(ctx)

    //  FIX 2: Asynchronous Delayed Cleanup
...
func checkResolver2(ctx context.Context, client *http.Client, cfg *runtimeConfig, resolver parsedResolver, port int) (Result, bool) {
	...
	// --- STEP 2: INTRA-TUNNEL FAST FAIL & QUICK SCAN (HANDSHAKE) ---
	// If EngineQuickScan is enabled, we ALWAYS perform the handshake to verify light e2e connectivity.
...
//	tunnelMu.Unlock()
	...
	if !cfg.SkipQuickCheck {
		// If the server doesn't respond to a basic DNS query within the timeout, skip it
...
//	tunnelCtx, cancelTunnel := context.WithCancel(ctx)
//	defer cancelTunnel() // Instantly signals shutdown. run.go handles the 500ms safety delay.
...
func verifyTunnelHandshake(ctx context.Context, port int, tCfg bridge.TunnelConfig) bool {
	...
	//   HTTP Proxy Handshake Check
	if protocol == "http" || protocol == "https" {
		// Send standard HTTP CONNECT greeting
		req := "CONNECT www.google.com:443 HTTP/1.1\r\nHost: www.google.com:443\r\n\r\n"
		if _, err := conn.Write([]byte(req)); err != nil {
			return false
		}

		buf := make([]byte, 15) // We just need enough bytes to read "HTTP/1.1 200 OK"
		conn.SetReadDeadline(time.Now().Add(2 * time.Second))
		n, err := conn.Read(buf)
		if err != nil || n < 12 {
			return false
		...
		// Success if the proxy server returns a 200 Connection Established
...
/*
func verifyTunnelHandshake(ctx context.Context, port int) bool {
	dialer := net.Dialer{Timeout: 3 * time.Second}
	conn, err := dialer.DialContext(ctx, "tcp", fmt.Sprintf("127.0.0.1:%d", port))
	if err != nil {
		return false
	...
	// 1. Send SOCKS5 Greeting (\x05 version, \x01 methods, \x00 no auth)
	_, err = conn.Write([]byte{0x05, 0x01, 0x00})
	if err != nil {
		return false
	}

	// 2. Wait for response (\x05 version, \x00 method accepted)
	buf := make([]byte, 2)
	conn.SetReadDeadline(time.Now().Add(2 * time.Second))
	n, err := conn.Read(buf)
	
	// FINAL CHECK: Ensure we got exactly 2 bytes and they match SOCKS5 success
	if err != nil || n != 2 {
		return false
	}
